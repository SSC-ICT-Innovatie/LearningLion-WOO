...
def select_woogle_dump_folders(path:str='./docs') -> str:
    ...
    if not folders_with_woogle_dumps:
        print("No folders containing 'WoogleDumps' were found.")
        return None
...
def read_docs(path:str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    document_dtypes = {'dc_identifier': str, 'foi_dossierId': str, 'dc_title': str, 'foi_fileName': str, 'dc_format': str, 'dc_source': str, 'dc_type': str, 'foi_nrPages': float}
    
    # Start recording the time
    start_time = time.time()
    print(f"Reading data from {path}...")
    
    # Get all the bodytext dataframes, because they are split over multiple files
    files = glob.glob(f'{path}/woo_bodytext*.csv.gz')
    dataframes = []
    for file in files:
        df = pd.read_csv(file, dtype=bodytext_dtypes).set_index('foi_documentId')
        dataframes.append(df)
    bodytext_dataframe = pd.concat(dataframes)
    ...
    # Log time it took
    print(f"Data read in {time.time() - start_time:.2f} seconds.")
...
def truncate_merge_docs(length:int=-1) -> None:
    """
    ...
    if length == -1:
      length = nr_dossiers  
    elif length > nr_dossiers:
        print(f"Length is larger than the number of dossiers in the dataset. Exiting...")
        return None
    ...
    print(f"Truncated data successfully saved to {new_directory}.")
