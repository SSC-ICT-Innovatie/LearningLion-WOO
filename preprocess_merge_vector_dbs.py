...
def merge_in_batches(target_store, source_data, max_id, batch_size=5461):
    ...
    print("num_batches: ", num_batches, flush=True) 
    ...
    for i in range(num_batches):
        ...
        print("max batch id: ", batch_ids[-1], flush=True)
...
def main():
    ...
    for index, (collection_name, vector_db_folder) in enumerate(folder_tuple):
        if index < 42: # Temp fix to only merge after part 42, because that has been done before.
            continue
        ...
        merge_in_batches(main_vector_store, vector_store_to_add_data, max_id)
