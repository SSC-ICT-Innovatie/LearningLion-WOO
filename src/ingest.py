...
class Ingester:
    def __init__(
        ...
        self.text_splitter_method = (
            settings.TEXT_SPLITTER_METHOD
            if text_splitter_method is None
            else text_splitter_method
        )
...
def main():
    ...
    if args.content_folder_name:
        ...
        content_folder_name = utils.get_content_folder_name(
            only_check_woo=settings.DATA_TYPE == "woo"
        )
    logger.info(f"Source folder of documents: {content_folder_name}")

    # get associated source folder path and vectordb path
    content_folder_path, vectordb_folder_path = utils.create_vectordb_name(
        content_folder_name
    )
    ingester = Ingester(
